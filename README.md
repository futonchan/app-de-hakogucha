...
- ゲーム開始時は最下段2行に、10パターンから選んだパンチでは壊せない初期箱を配置します。
- 十字キーで上下左右移動／隣接する静止箱のプッシュ、パンチで向き方向の隣接箱を攻撃します。箱へのアクション後もプレイヤーは自動前進せず、押しっぱなし入力も空いたマスへの通常移動へ引き継ぎません。
- 落下・プッシュ中の表示座標とグリッド座標を分離し、移動中の箱は連結消去から外し、移動完了後に判定へ戻します。
- 消去対象の箱は0.5秒点滅してから削除され、その間は生成・落下・操作・制限時間・コンボ判定のワールド時間を停止します。
