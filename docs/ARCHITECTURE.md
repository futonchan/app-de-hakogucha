...
3. 支えを再計算し、停止中の同色連結を検出・一括消去し、再び支えを更新する。
4. tに期限が来た箱の落下可能性を下から計算し、一括適用する。圧死ならその場で終了する。
5. 支えを更新し、着地などで生じた連結消去を処理する。
...
初回は250msごとの論理マス移動をそのまま表示してもよい。
補間アニメーションを加える場合は見た目と衝突マスの対応を説明し、
「まだ触れていないのに死ぬ」「既に通過しているのに生きる」と見える表示を避ける。
