...
type PlayerMotion = { fromX: number; fromY: number; toX: number; toY: number; startedAtMs: number; endsAtMs: number };
...
プレイヤーも`playerMotion`を持つ場合は、`player.x,y`を前マスの論理座標として保持し、`playerMotion.endsAtMs`で到達先へ論理座標を確定する。
描画座標は`playerMotion`から補間し、移動中に受けた次方向入力は`queuedMoveDirection`として1方向だけ保持し、到着直後に処理する。R-28。
...
プッシュ開始前に、押し先セルが静止箱の論理座標、プッシュ中箱の開始／到達セル、落下中箱の論理座標／次の落下先のいずれにも該当しないことを確認する。
この安全判定を通った場合だけ`motion`を設定し、落下アニメーション完了時に同一セルへ確定する箱同士の重なりを防ぐ。R-27。
...
2. tで完了するプレイヤー移動があれば到着先へ論理座標を確定し、待機方向があればその時点の盤面で移動／プッシュを試みる。
3. tの入力を処理する。同時刻内は`release`を先、移動系を次、パンチを後、その中では`seq`順とする。入力で開始したプレイヤー移動は`playerMoveStepMs`後の完了イベントとして扱う。
4. 支えを再計算し、移動中または同時刻に完了予定の箱を除いた停止中の同色連結を検出する。対象があれば`clearing`へ入り、この時刻のワールド処理を止める。
5. tに期限が来た落下アニメーションまたはプッシュアニメーションを確定する。落下確定で圧死ならその場で終了する。
6. 支えを更新し、着地・プッシュ完了などで生じた連結消去を検出する。対象があれば`clearing`へ入り、この時刻のワールド処理を止める。
7. tが生成期限なら、現在の盤面で生成する。空き列なし／生成マスのプレイヤー衝突なら終了する。
8. 支え・連結消去を更新する。対象があれば`clearing`へ入り、この時刻のワールド処理を止める。
9. 0.5秒の点滅完了後、`finishClearAnimation`で対象箱を削除し、コンボを1回だけ更新して加点し、最大コンボを更新する。
...
プレイヤーの移動は初期50msで1マスの等速補間として表示し、`playerMotion.endsAtMs`で論理座標を到着先へ確定する。R-28。
補間アニメーション中の表示座標は衝突・連結消去の正本にしない。
ただしプッシュ可否の衝突判定では、移動中の箱が表示上通過／到達するマスを予約セルとして扱い、同じマスや同じ表示位置へ別箱を押し込まない。R-27。
